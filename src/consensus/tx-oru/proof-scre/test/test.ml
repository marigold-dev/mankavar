...
module Structs = Oru_scre_raw.Structs
module Eval = Oru_scre_raw.Eval
...
let mk_bootstrap () =
  ...
  let lst =
    ...
    |> List.map Structs.Account_index.key_index
  in
...
let counter_contract =
  ...
  let c_program = [
    function_ "main" "_" [
      return @@ (
        Eval.C_like.write (literal 0L) @@
        add
          (Eval.C_like.read_data (literal 0L))
          (Eval.C_like.read (literal 0L))
      ) ;    
    ]
  ] in
  let program = Das_vm.Compile.compile_program c_program in
  let storage = Das_vm.VMap.of_list [(0L , 0L)] in
  Structs.Contract.make ~program ~storage
...
let test_simple_transfer_to_contract =
  ...
  assert (Eval.equal_memory ctr.storage Das_vm.VMap.(of_list [(0L ,15L)])) ;
  push_op @@ Operation.transfer @@
    Transfer.make_tpl a b 300L [|17L|] 100L ;
  let ctr = S.get_contract_exn 1L in
  assert (Eval.equal_memory ctr.storage Das_vm.VMap.(of_list [(0L ,32L)])) ;
  ()
