...
module Make(Params : Woo_helpers.PARAMS) = struct
  module Generate = struct
    ...
    let str ~prefixes ?wrap_constructor ?wrap_get ?wrap_map : (P.rec_flag * P.type_declaration list) -> P.structure_item list = fun (rec_flag , tds) ->
      ...
      let aux : W.type_declaration -> P.structure_item list = fun td ->
        let W.{ label ; ty ; non_recursive = _ ; parameters = _ } = td in
        let prefix =
          if label = "t"
          then ""
          else label ^ "_"
        ...
        | W.T_variant variant -> (
          let wrap_constructor =
            ...
            | Some (name , body) when name = label -> Some body (* once told me *)
            | _ -> None
          in
          let wrap_get =
            match wrap_get with
      ...
      let items = List.map declaration_of_prefix prefixes @ items in
      let items = List.filter (fun x -> not @@ Helpers.is_tauto_type_declaration x) items in
      items
...
let deriver =
  let str_type_decl =
    let generate ~loc ~path:_ input prefix prefixes wrap_constructor wrap_get wrap_map =
      let prefixes =
        let prefix =
          match prefix with
          | Some x -> [ x ]
        ...
        prefix @ prefixes
      in
      let module Full = Make(val Woo_helpers.make_PARAMS loc) in
      Full.Generate.str ~prefixes ?wrap_constructor ?wrap_get ?wrap_map input
    in
