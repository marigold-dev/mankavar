...
let property_type : W.labelled_record_field -> P.structure_item list = fun lt ->
  let (label , W.{ type_expression = ty ; default_value = _ ; index = _  ; parameters }) = lt in
...
let property : W.labelled_record_field -> P.structure_item list = fun lts ->
  let (label , W.{ type_expression = ty ; default_value = _ ; index = _ ; parameters = _ }) = lts in
  ignore ty ;
  let body =
...
let mappers : W.record -> P.structure_item list = fun property_declarations ->
  let ltss = SMap.to_kv_list property_declarations in
  let itemss = List.map mapper ltss in
  let items = List.concat itemss in
  items
...
let setters : W.record -> P.structure_item list = fun property_declarations ->
  let ltss = SMap.to_kv_list property_declarations in
  let itemss = List.map setter ltss in
  let items = List.concat itemss in
  items
