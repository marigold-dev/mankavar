...
let matching ?wrap_get ~namer case : W.variant -> W.constructor_declaration -> P.structure_item = fun variant (base_label , _ty) ->
  ...
  let body =
    match wrap_get with
    | None -> A.pexp_function cases
    | Some wrap -> (
      let lhs =
        A.pexp_apply wrap [
          (Nolabel , (A.pexp_ident @@ lident "x")) ;
        ]
      in
      A.pexp_fun Nolabel None (p_var "x") @@
      A.pexp_match lhs cases
    )
  in
  let declaration = A.value_binding ~expr:body ~pat:function_name in
    ...
    if l = 0 then (
      ...
      let pattern =
        ...
        then p_var variable_name
          ...
          let pattern_names = List.init l p_tuple_name in
          let patterns = List.map (fun name -> A.pvar name) pattern_names in
  ...
  let rhs =
    ...
    let local_lhs_expr =
      if l = 0 then (
        ...
        let var_names = List.init l p_tuple_name in
    ...
    in
    A.pexp_let P.Nonrecursive [ lhs ] rhs
  in
  A.case ~lhs ~guard:None ~rhs

let matching_full ?wrap_get ~namer rhs : W.variant -> P.structure_item list = fun variant ->
  let matching_single : W.variant -> W.constructor_declaration -> P.structure_item =
...
let matching_exn_full ?wrap_get : W.variant -> P.structure_item list =
  ...
  let rhs ~base_label ~current_label ~variable =
    let failure =
      ...
      e_apply
        (e_var "raise")
        (e_constructor ~polymorphic:false "Failure" ~body:str_constant)
    in
...
let matching_opt_full ?wrap_get : W.variant -> P.structure_item list =
  ...
  let rhs ~base_label ~current_label ~variable =
    if String.equal base_label current_label
    then e_constructor ~polymorphic:false "Some" ~body:variable
    else e_constructor ~polymorphic:false "None"
  in
